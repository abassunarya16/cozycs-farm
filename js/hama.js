var hama = (function() {
    ...
    function render() {
        ...
        if (data.length === 0) {
            ...
            data.forEach(function(h) {
                var tingkatBg = '#E3F2FD';
                var tingkatColor = '#1976D2';
                if (h.tingkat === 'Berat') { tingkatBg = '#FFEBEE'; tingkatColor = '#D32F2F'; }
                else if (h.tingkat === 'Sedang') { tingkatBg = '#FFF3E0'; tingkatColor = '#F57C00'; }
                ...
                html += '<div><strong style="font-size:14px; color:#1B5E20;">' + (h.jenis || '-') + '</strong><br><small style="color:#888;">' + (h.tanaman_id || '-') + '</small></div>';
    ...
    function showForm(id) {
        ...
        content += '<h3 style="margin:0;font-size:16px;"><i class="fas '+(isEdit?'fa-edit':'fa-plus-circle')+'" style="margin-right:8px;color:#2E7D32;"></i>' + (isEdit?'Edit':'Input') + ' Hama/Penyakit</h3>';
        content += '<button onclick="hama.closeModal()" style="background:none;border:none;font-size:20px;color:#999;cursor:pointer;">✕</button></div>';
