var tanaman = (function() {
    ...
    function render() {
        ...
        if (data.length === 0) {
            ...
            data.forEach(function(t) {
                ...
                html += '<button onclick="tanaman.deleteData(\'' + t.id + '\')" style="padding:6px 12px;background:#FFEBEE;color:#D32F2F;border:none;border-radius:6px;font-size:11px;cursor:pointer;"><i class="fas fa-trash-alt"></i> Hapus</button>';
                html += '</div>';
    ...
    function deleteData(id) {
        if (confirm('Hapus tanaman ' + id + '?\n\nData terkait (polinasi, buah, panen) juga akan dihapus.')) {
