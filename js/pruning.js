var pruning = (function() {
    ...
    function render() {
        ...
        if (data.length === 0) {
            ...
            html += '<i class="fas fa-cut" style="font-size:40px; color:#ddd; margin-bottom:12px;"></i><h3 style="color:#777; font-size:16px;">Belum ada riwayat pruning</h3></div>';
        } else {
            html += '<div style="font-size:14px; font-weight:700; color:#333; margin-bottom:12px; display:flex; align-items:center; gap:8px;">';
            html += '<i class="fas fa-history" style="color:#777;"></i> Riwayat Perawatan</div>';
            ...
            data.forEach(function(p) {
                ...
                html += '<div style="font-size:11px; color:#F57C00; background:#FFF3E0; padding:4px 8px; border-radius:6px; font-weight:700; text-transform:uppercase;">' + (p.jenis_pruning || '-') + '</div>';
                html += '</div>';
                ...
                html += '<button onclick="pruning.editForm(\'' + p.id + '\')" style="padding:6px 14px; background:#E3F2FD; color:#1976D2; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;"><i class="fas fa-edit"></i> Edit</button>';
                html += '<button onclick="pruning.deleteData(\'' + p.id + '\')" style="padding:6px 14px; background:#FFEBEE; color:#D32F2F; border:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer;"><i class="fas fa-trash-alt"></i> Hapus</button>';
                html += '</div>';
    ...
    function showForm(id) {
        ...
        html += '<h3 style="margin:0;font-size:16px; color:#2E7D32;"><i class="fas ' + (isEdit ? 'fa-edit' : 'fa-plus-circle') + '" style="margin-right:8px;"></i>' + (isEdit ? 'Edit' : 'Input') + ' Pruning</h3>';
        ...
        var jP = data.jenis_pruning === 'Pucuk' ? 'selected' : '';
        var jC = data.jenis_pruning === 'Cabang' ? 'selected' : '';
        var jD = data.jenis_pruning === 'Daun' ? 'selected' : '';
        var jS = data.jenis_pruning === 'Sulur' ? 'selected' : '';
        html += '<div><label style="display:block;font-size:12px;font-weight:600;color:#555;margin-bottom:6px;">Jenis Pruning</label><select name="jenis_pruning" style="width:100%;padding:12px;border:1.5px solid #ddd;border-radius:8px;font-size:14px;background:#fff;outline:none;"><option '+jP+'>Pucuk</option><option '+jC+'>Cabang</option><option '+jD+'>Daun</option><option '+jS+'>Sulur</option></select></div>';
        html += '</div>';
        ...
        html += '<button type="button" onclick="pruning.closeModal()" style="padding:12px 20px;background:#f5f5f5; color:#555; border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;">Batal</button>';
    ...
    function deleteData(id) {
        if (confirm('Yakin ingin menghapus riwayat pruning ini?')) {
            Storage.delete(Storage.KEYS.PRUNING, id);
            Router.navigate('pruning');
            if(typeof Notification !== 'undefined') Notification.success('Data pruning dihapus!');
        }
    }
    ...
    function save(event, id) {
        ...
        closeModal();
        Router.navigate('pruning');
        if(typeof Notification !== 'undefined') Notification.success('Data pruning disimpan!');
    }
